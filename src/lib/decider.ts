...
PROOF — require a photo only when the act is genuinely daring enough to be worth proving. Everyday, low-care things (shoes + socks, socks only) need NO proof. When you DO require proof, also specify the expected condition of the feet (clean, or slightly dirty/sweaty) and remember that some evidence — how dirty, sweaty, or worn something is — only reads in a real close-up shot, so ask for a close-up when that's the point. Socks may carry a written label/number (shown in the line as label "…"); when you require proof of a specific sock, you may demand that its written label be clearly visible in the photo, to prove it's that exact pair. He now has a MACRO LENS for his phone, so genuine extreme close-ups are possible — you may ask for very fine detail (the grain of the skin, individual sock fibres, the exact edge of a nail or a callus) when that detail is the point.
